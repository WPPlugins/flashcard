...
/*  Copyright 2013 Liang Shao  (email : curious.furious@gmail.com)
 ...
 */
wp_enqueue_script('jquery-css-transform', WP_PLUGIN_URL.'/flashcard/rotate3Di/jquery-css-transform/jquery-css-transform.js', array('jquery'));
...
function parseUTF8Text($filename){
	...
	curl_setopt($cp, CURLOPT_URL, $filename);
...
function flashcard_initialize($attr){
	...
	$i = 0;
	foreach ($result as $r){
		$fc_content .= '<li style="width:'.$width.'px;height:'.$height.'px;">';
		...
		$fc_content .= '<div class="front" style="width:'.$width.'px;height:'.$height.'px;background:'.$front_bg_color.';"><div style="font-size:'.$front_text_size.'px;color:'.$front_text_color.';">'.$front.'</div></div>';
		$fc_content .= '<div class="back" style="width:'.$width.'px;height:'.$height.'px;background:'.$back_bg_color.';"><div style="font-size:'.$back_text_size.'px;color:'.$back_text_color.';">'.$back.'</div></div>';
		$fc_content .= '</li>';
